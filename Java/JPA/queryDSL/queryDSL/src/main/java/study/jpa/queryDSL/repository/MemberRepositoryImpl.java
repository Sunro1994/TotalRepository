...
import static study.jpa.queryDSL.domain.QMember.member;
import static study.jpa.queryDSL.domain.QTeam.team;

// 해당레포지터리 이름 + Impl이라는 이름으로 만들어야한다. 규칙임
public class MemberRepositoryImpl implements  MemberRepositoryCustom{
    ...
    public Page<MemberTeamDto> searchPagecomplex(MemberSearchCondition condition, Pageable pageable) {
        List<MemberTeamDto> content = queryFactory
                ...
                .offset(pageable.getOffset())
                .limit(pageable.getPageSize())
                .fetch(); //최적화를 하고 싶으면 이렇게 별도로 쪼개야한다. fetchresult를 사용하면 모든걸 다 가져오기 떄문
