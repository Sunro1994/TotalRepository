...
private static void l_pivot_sort(int[] a, int lo, int hi) {
	 ...
	 * 만들어 준뒤, 최종적으로 pivot의 위치를 얻는다.
	 * 
	 * 그리고 나서 해당 피벗을 기준으로 왼/오 부분리스트로 나누어 분할 정복을 해준다.
	 * 
	 * [과정]
	 *  Partitioning:
		 *
...
private static int partition(int[] a, int left, int right) {
	...
	while(lo < hi) {
		/*
		 * hi가 lo보다 크면서, hi의 요소가 pivot보다 작거나 같은 원소를 찾을 때까지 hi감소
		 */
		while(a[hi]>pivot && lo<hi) hi--;
		
		/*
		 * hi가 lo보다 크면서, lo의 요소가 pivot보다 큰 원소를 찾을때까지 lo를 증가시킨다.
		 * 
		 */
	
		while(a[lo]<=pivot && lo<hi) lo++;
		
//		교환 될 두 요소를 찾았으면 요소를 교환
		swap(a,lo,hi);
	}
		
		/*
		 * 마지막으로 맨 청므 pivot으로 설정했던 위치(a[left])의 원소와 lo가 가키리는 원소를 바꾼다.
		 */
		swap(a,left,lo);
